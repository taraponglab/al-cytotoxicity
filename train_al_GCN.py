...
class MolecularDataset(Dataset):
    """Custom PyTorch Dataset for GCN-based molecular data."""
...
class GCNModel(nn.Module):
    ...
    def forward(self, graph_data):
        # --- Encoder ---
        latent_features = self.gnn(graph_data)
    ...
    def predict_proba(self, graph_data):
        ...
        with torch.no_grad():
            logits, _ = self.forward(graph_data)
...
# === 5. Training Function ===
def train_model(model, train_loader, criterion_cls, optimizer, device):
    """Train GCNModel for one epoch"""
    model.train()
    total_loss = 0
    correct = 0
    total = 0
...
def train_with_validation(model, full_train_dataset, epochs, criterion_cls, optimizer, scheduler, device):
    ...
    if len(full_train_dataset) < 5: # Cannot split if too small
        ...
        for epoch in range(epochs):
            loss, acc = train_model(model, train_loader, criterion_cls, optimizer, device)
    ...
    for epoch in range(epochs):
        loss, acc = train_model(model, sub_train_loader, criterion_cls, optimizer, device)
        history['loss'].append(loss)
        history['acc'].append(acc)

        # Evaluate on the sub-validation set
        val_probs, val_labels = evaluate_model(model, sub_val_loader, device)
...
def evaluate_model(model, data_loader, device):
    """Evaluate GCNModel and return predictions"""
    model.eval()
    all_probs = []
    all_labels = []
...
def get_latent_embeddings(model, dataset, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # Encoder pass
            _, latent_features = model(graph_data)
...
def uncertainty_sampling(model, pool_dataset, n_samples: int, device) -> np.ndarray:
    ...
    probs, _ = evaluate_model(model, pool_loader, device)
...
def run_initial_model_evaluation(
    ...
    # Create datasets
    initial_train_dataset = MolecularDataset(smiles_train[initial_idx], y_train[initial_idx])
    pool_dataset = MolecularDataset(smiles_train[pool_idx], y_train[pool_idx])
    test_dataset = MolecularDataset(smiles_test, y_test)
    ...
    pool_probs, pool_labels = evaluate_model(model, pool_loader, device)
...
def run_al_experiment_for_strategy(
    ...
    initial_dataset = MolecularDataset(smiles_train[initial_idx], y_train[initial_idx])
    ...
    for i in range(n_queries):
        ...
        labeled_dataset = MolecularDataset(smiles_train[labeled_idx], y_train[labeled_idx])
        pool_dataset = MolecularDataset(smiles_train[pool_idx], y_train[pool_idx])
        ...
        pool_probs, _ = evaluate_model(model, pool_loader_for_sampling, device)
        ...
        updated_labeled_dataset = MolecularDataset(smiles_train[labeled_idx], y_train[labeled_idx])
...
def active_learning_gcn(
    ...
    """Run active learning experiment with GCNModel, starting from a pre-defined split."""
...
def main(random_seed=0):
    print(f"\n{'='*80}")
    print(f"🚀 Starting Active Learning with GCN model")
    print(f"SEED: {random_seed}")
    print(f"{'='*80}\n")
    
    # Load data
    print("📂 Loading data...")
    output_dir = f"active_learning_GCN_{random_seed}"
    ...
    initial_idx = np.random.choice(len(y_train), n_initial, replace=False)
    pool_idx = np.setdiff1d(np.arange(len(y_train)), initial_idx)
    ...
    # Run active learning
    test_perf, cumulative_hits, sample_sizes = active_learning_gcn(
        smiles_train, y_train,
        smiles_test, y_test,
        initial_idx, pool_idx,
        output_dir=output_dir,
    ...
    print(f"\n{'='*80}")
    print("✅ GCN Experiments Completed!")
    print(f"{'='*80}\n")
